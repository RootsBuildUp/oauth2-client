...
import com.RootsBuildup.OAuth2Client.service.AccessTokenService;
...
public class CustomTokenStore implements TokenStore {
	...
	public OAuth2Authentication readAuthentication(OAuth2AccessToken token) {
		...
		if(username != null && clientId != null){
			OAuthToken oAthToken = accessTokenService.getOAuthToken(username+clientId);
			if(token.getValue().equals(oAthToken.getToken())) {
				accessTokenService.asyncMethodForTokenTimeUpdate(oAthToken);
				return getOAuth2Authentication(token);
			}
			else return null;
		}
		return null;
	...
	private OAuth2Authentication getOAuth2Authentication(OAuth2AccessToken token){
		DecodedJWT jwt = com.auth0.jwt.JWT.decode(token.getValue());
	...
	public OAuth2AccessToken readAccessToken(String tokenValue) {
		...
		if(username != null && clientId != null){
			...
			String id = username+clientId;
			log.info("it came here readAccessToken");
			OAuthToken oAthToken = accessTokenService.getOAuthToken(id);
